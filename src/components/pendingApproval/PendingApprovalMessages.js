...
export const PendingApprovalMessages = () => { // Ensure the component name is capitalized
                ...
                try {
                    ...
                    await Promise.all(
                        propertiesSnapshot.docs.map(async (property) => {
                            const pendingApprovalRef = collection(db, "users", user.email, "properties", property.id, "PendingApproval");
                    ...
                    await setDoc(doc(db, "users", user.email), { pendingApprovalMessage: true }, { merge: true });
                } catch (error) {
