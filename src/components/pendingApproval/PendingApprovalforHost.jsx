          ...
          const propertyMap = await Promise.all(
            propertySnapshot.docs.map(async (property) => {
              const propertyData = await getDocs(
                collection(db, 'users', user.email, 'properties', property.id, 'PendingApproval'),
              );
  ...
  const deleteRequest = async (pendingProperty, pendingRequestID) => {
    ...
    try {
      ...
      setPendingProperties(
        (
          prev, //prev: Refers to the current state of pendingProperties
        ) =>
          prev.map((property) => {
            ...
            if (property?.id && pendingProperty?.id && property.id === pendingProperty.id) {
              return {
                ...property,
                availability:
                  property.availability?.filter((req) => req.id !== pendingRequestID) || [],
              };
            }
            ...
            <div>
              {pendingProperty.availability.map((pending, index) => (
                <div key={index} className={styles.center}>
                  ...
                  <div className={styles.row}>
                    <Button onClick={() => approveRequest(pendingProperty, pending.id)}>
                      Approve
                    </Button>
                    <Button onClick={() => deleteRequest(pendingProperty, pending.id)}>
                      Disapprove
                    </Button>
                  </div>
                </div>
