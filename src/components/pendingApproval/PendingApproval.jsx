...
import { PendingApprovalforRenter } from './PendingApprovalforRenter.jsx';
import { PendingApprovalforHost } from './PendingApprovalforHost.jsx';
import { PendingApprovalMessages } from './PendingApprovalMessages.jsx';
...
export const PendingApproval = () => {
  ...
  const filterPendingPorpertiesData = pendingPropertiesData.filter(
    (property) => property.pendingApprovals[0]?.requestingUser === user.email,
  );
      ...
      try {
        ...
        await Promise.all(
          querySnapshot.docs.map(async (user) => {
            ...
            await Promise.all(
              checkProperties.docs.map(async (property) => {
                //getting me docs from properties
