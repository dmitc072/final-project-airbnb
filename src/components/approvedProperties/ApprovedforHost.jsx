        ...
        if (!propertySnapshot.empty) {
          ...
          const propertyMap = await Promise.all(
            propertySnapshot.docs.map(async (property) => {
              const propertyData = await getDocs(
                collection(db, 'users', user.email, 'properties', property.id, 'Approval'),
              );
    ...
    <>
      <div className={styles.container}>
        <Typography sx={{ fontWeight: 'bold' }} variant="h5">
          Approved Property Request to Host
        </Typography>
        <Typography variant="h5" className={styles.margin}>
          Total Earnings: ${investment}
        </Typography>
                ...
                <div key={index} className={styles.center}>
                  <div className={styles.row}>
                    ...
                    <div className={styles.button}>
                      <Button
                        disabled={!approved.reviewMessage}
                        sx={{
                          '&.MuiButton-root.Mui-disabled': {
                            color: 'grey',
                          },
                        }}
                        onClick={() => reviewMessage(approvedProperty, approved.reviewMessage)}
                      >
                        {approved.reviewMessage ? 'See Review' : 'No Review'}
                      </Button>
                    </div>
                  </div>
                </div>
